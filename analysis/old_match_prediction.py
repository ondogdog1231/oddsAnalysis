...
leagueModels = c.getPredictionModelByLeagueId(league_id)

date_string = '2023-05-27 22:00:00'
date_obj = datetime.datetime.strptime(date_string, '%Y-%m-%d %H:%M:%S')
unix_timestamp = int(time.mktime(date_obj.timetuple()))

matchDetails = c.getMatchByBetweenTimeAndSeasonAndStarted(league_id, unix_end_time, ["2023-2024", "2022"])
...
for model in leagueModels:
    contrary = False if model[5] == 0 else True
    enable = False if model[6] == 0 else True
    modelName = model[2]
    predictionList[modelName] = {
        "prediction": None,
        "predict_proba": None,
        "contrary": contrary,
        "confidence_level": model[4]
    }
    # enable?
    if enable is not True:
        continue
    clf = joblib.load(f"../oldMatchPredictionModels/{league_id}_{modelName}_model.pkl")
    predictionList[modelName]["prediction"] = clf.predict(df)
    predictionList[modelName]["predict_proba"] = clf.predict_proba(df)
...
#         continue
#     clf = joblib.load(f"../predictionModels/old_{league_id}_{model['name']}_model.pkl")
...
for index, match_id in enumerate(filtered_predictionMatchList):
    ...
    for modelName, predictionDetails in predictionList.items():
        ...
        #      modelName), None)
        confidence_level = predictionDetails["confidence_level"]
        ...
        prediction_found = c.checkOldHistoryPrediction(matchDetails[index][0], modelName)
        ...
        #     exit()
        if prediction_found:
            c.updateOldMatchPredictionNet(prediction_found[0][0], str(predictionDetails["prediction"][index]), predict_proba,
                                  net)
            continue
        c.insertOldMatchPrediction(insertParams)
